# app.py — SoNo server (clean init + profiles preload + pam.txt preload)
...
from retriever_pam import init_pam_retriever , retrieve_pam_answer
...
from profiles import load_profiles, profile_answer # <-- plural loader
from ingest_pam import load_pam_facts # <-- pam.txt -> dict
...
# --- Initialize semantic retriever for pam.txt ---
PAM_RET = None
try:
    PAM_RET = init_pam_retriever(Path("data/pam.txt"))
    app.logger.info("Pam retriever ready")
except Exception as e:
    app.logger.warning(f"Pam retriever init failed: {e}")

# ------------------------------------------------------------------------------
# Load profiles (data/mom_profile.json, data/sono_profile.json)
# and seed their facts into MemoryStore
# ------------------------------------------------------------------------------
try:
    PROFILES = load_profiles(Path("data")) # expects a *directory*
    # seed each profile's facts into MemoryStore
    for subj, prof in PROFILES.items():
        facts = prof.get("facts") or {}
        for rel, val in facts.items():
            try:
                store.remember(subj, str(rel).strip().lower(), str(val).strip())
            except Exception as e:
                app.logger.warning(f"Seed warn ({subj}|{rel}): {e}")
    app.logger.info(f"Profiles loaded: {', '.join(PROFILES.keys())}")
except Exception as e:
    PROFILES = {}
    app.logger.warning(f"Profiles load failed: {e}")

# ------------------------------------------------------------------------------
# Load pam.txt facts (normalized Q/A pulled from data/pam.txt) and seed them too
# ------------------------------------------------------------------------------
try:
    pam_facts = load_pam_facts(Path("data/pam.txt")) # returns {relation -> value}
    for rel, val in pam_facts.items():
        try:
            store.remember("pam", str(rel).strip().lower(), str(val).strip())
        except Exception as e:
            app.logger.warning(f"Pam fact seed warn ({rel}): {e}")
    app.logger.info(f"Pam facts loaded: {len(pam_facts)}")
except Exception as e:
    app.logger.warning(f"PAM ingest skipped: {e}")

# ------------------------------------------------------------------------------
# Light Q&A patterns that map directly to (subject, relation)
# ------------------------------------------------------------------------------
_QA_PATTERNS = [
...
def _heuristic_ask(t: str):
    """
    Very light parser:
      - catches possessives like "Pam's birthday"
      - alias-normalizes subject ("mom", "mother" -> "pam" when appropriate)
      - returns (subject, relation) or (None, None)
    """
    import re as _re
    t = (t or "").strip()

    # 1) direct patterns (who is ty's mom, what's pam's full name, etc.)
    for rx, pair in _QA_PATTERNS:
        if rx.search(t):
            return pair
    ...
    # 2) possessive-only style: "pam's birthday", "pam's favorite restaurants"
    m = _re.search(r"([a-z][a-z'\s]+?)'s\s+([a-z][a-z\s\-]+)$", low)
    if m:
        subj_raw = m.group(1).strip()
        rel_raw = m.group(2).strip()
        subj = subj_raw.replace("’", "'").strip()
        rel = rel_raw.replace("’", "'").strip()

        # normalize known aliases
        subj_norm = subj
        if subj in ("mom", "mother", "ty's mom", "ty’s mom", "tys mom"):
            subj_norm = "pam"
        elif subj in ("sono", "soul", "soul node", "soulnode"):
            subj_norm = "sono"

        return (subj_norm, rel)

    # 3) simple "<sub>'s <rel>?" phrasing without the apostrophe char variations handled above
    m2 = _re.search(r"\b([A-Za-z][A-Za-z ]+)\s+is\s+([A-Za-z][A-Za-z \-]+)\??$", low)
    if m2:
        # very rough; keep for edge text like "Pam is favorite teacher?"
        subj_norm = m2.group(1).strip().lower()
        rel = m2.group(2).strip().lower()
        if subj_norm in ("mom", "mother", "ty's mom", "ty’s mom", "tys mom"):
            subj_norm = "pam"
        return (subj_norm, rel)

    # 4) fallback: nothing clean detected
    return (None, None)
...
# ====== handler: pro routing (identity → memory → profile_facts → GPT) ======
# Requires: from flask import request
# from profiles import profile_answer
# store.remember/store.update/store.forget/store.recall
# parse_intent, _looks_dangerous, get_last_context, set_last_context, gpt_bridge
...
# one source of truth for names/relations (used by BOTH parse & routing)
_SUB_ALIASES = {
    "ty": {"ty", "ty butler"},
    "pam": {"pam", "pamlea", "pam leah", "ty's mom", "tys mom"},
    "sono": {"sono", "soulnode", "soul node", "assistant", "you"},
    # add others here as you grow
}

_REL_ALIASES = {
    "mom": {"mom", "mother"},
    "full name": {"full name", "fullname", "name"},
    "hometown": {"hometown", "home town", "where raised", "raised", "grew up"},
    "birthplace": {"birthplace", "born in", "place of birth", "where born"},
    "favorite team":{"favorite team", "fav team"},
    "favorite color":{"favorite color", "fav color"},
    # add more canonical relations here
}

def _canon_subject(s: str|None) -> str|None:
    if not s: return None
    s = s.strip().lower()
    for canon, variants in _SUB_ALIASES.items():
        if s in variants or s == canon:
            return canon
    return s # fall through (still lowercased)

def _canon_relation(r: str|None) -> str|None:
    if not r: return None
    r = r.strip().lower()
    for canon, variants in _REL_ALIASES.items():
        if r in variants or r == canon:
            return canon
    return r

# --- Handle user text (main router) ---
def handle_user_text(user_text: str):
    """
    Unified handler with robust memory routing:
      - guards
      - identity answers
      - intent parse + context capture
      - memory-first (with aliases + multi-key fallback)
      - GPT fallback
    Returns {ok, source, response}
    """
    try:
        ...
        if len(txt) > 1200:
            return {"ok": False, "source": "guard", "response": f"Input too long ({len(txt)} chars). Keep it under 1200."}
        if _looks_dangerous(txt):
            return {"ok": False, "source": "guard", "response": "Potentially dangerous input blocked."}

        # ---------- Identity first (never send to GPT) ----------
        try:
        ...
        # ---------- Intent parse ----------
        try:
            kind, sub, rel, obj = parse_intent(txt) # expected: ("teach"/"update"/"forget"/"ask"/"other", sub, rel, obj)
        except Exception:
            kind, sub, rel, obj = ("ask", None, None, None)

        # ---------- Quick pattern route (who is ty's mom, etc.) ----------
        subj_hint, rel_hint = (None, None)
        try:
            subj_hint, rel_hint = quick_qna_route(txt)
        except Exception:
            pass

        # ---------- Heuristic route (Pam's birthday, etc.) ----------
        if not subj_hint and not rel_hint:
            try:
                subj_hint, rel_hint = _heuristic_ask(txt)
            except Exception:
                subj_hint, rel_hint = (None, None)

        # Normalize subject aliases
        def _norm_sub(s: Optional[str]) -> Optional[str]:
            if not s: return s
            s = s.strip().lower()
            if s in {"mom", "mother", "ty's mom", "ty’s mom", "tys mom"}: return "pam"
            if s in {"pamlea", "pam butler", "pamela", "pamela butler"}: return "pam"
            if s in {"soulnode", "soul node", "soul", "sono"}: return "sono"
            if s in {"ty butler"}: return "ty"
            return s

        sub = _norm_sub(sub or subj_hint)
        rel = (rel or rel_hint or "").strip().lower() or None

        # ---------- Follow-up teach: "it's X / it is X / no, it's X / yes, it's X" ----------
        if low.startswith(("it's ", "it’s ", "it is ", "no, it's ", "no, it’s ", "yes, it's ", "yes, it’s ")):
            try:
                last = get_last_context(request.remote_addr) or {}
            except Exception:
                last = {}
            if last.get("sub") and last.get("rel"):
                # extract value after the lead-in phrase
                value = txt
                for p in ("it's ", "it’s ", "it is ", "no, it's ", "no, it’s ", "yes, it's ", "yes, it’s "):
                    if value.lower().startswith(p):
                        value = value[len(p):]
                        break
                value = value.strip().strip(".").strip()
                store.update(last["sub"], last["rel"], value)
                try:
                    set_last_context(request.remote_addr, None, None)
                except Exception:
                    pass
                return {"ok": True, "source": "teach", "response": f"Saved: {last['sub'].title()}|{last['rel']} → {value}"}

        # ---------- Explicit memory writes ----------
        if kind == "teach":
            store.remember(_norm_sub(sub), rel, obj)
            return {"ok": True, "source": "teach", "response": f"Got it — {(_norm_sub(sub) or '').title()}'s {rel} is {obj}."}
        if kind == "update":
            store.update(_norm_sub(sub), rel, obj)
            return {"ok": True, "source": "teach", "response": f"Updated — {(_norm_sub(sub) or '').title()}'s {rel} is now {obj}."}
        if kind == "forget":
            ok = store.forget(_norm_sub(sub), rel)
            return {"ok": True, "source": "teach", "response": f"Deleted — {(_norm_sub(sub) or '').title()}'s {rel}."} if ok \
                   else {"ok": False, "source": "teach", "response": "Nothing to delete."}

        # ---------- Record context for follow-up teach ----------
        try:
            if sub and rel:
                set_last_context(request.remote_addr, sub, rel)
            else:
                set_last_context(request.remote_addr, None, None)
        except Exception:
            pass

        # ---------- ROBUST MEMORY LOOKUP ----------
        # Build candidate keys that match how facts were stored
        def _cands(s: Optional[str], r: Optional[str]):
            s = _norm_sub(s) if s else s
            r = (r or "").strip().lower() or None
            c = []
            if s and r:
                c.append((s, r))
            # common relation synonyms
            if r in {"name", "fullname"}:
                c.append((s, "full name"))
            if r in {"birth place", "place of birth", "born"}:
                c.append((s, "birthplace"))
            if r in {"raised", "from"}:
                c.append((s, "hometown"))
            # special Ty→Pam mapping for mom
            if (s in {"ty", "ty butler"} or "ty" in (s or "")) and (r in {"mom", "mother"}):
                c.append(("pam", "full name")) # sometimes stored as full name; we’ll phrase it cleanly below
                c.append(("pam", "mom"))
            # generic fallbacks if we only got subject from heuristics
            if s and not r:
                c.append((s, "full name"))
                c.append((s, "hometown"))
                c.append((s, "birthplace"))
        ...
        ans = None
        used_key = None
        for ksub, krel in _cands(sub, rel):
            try:
                val = store.recall(ksub, krel)
            except Exception:
                val = None
            if val:
                ans = val
        ...
        # If memory hit, answer in a *sentence*, not “just the value”
        if ans:
            subj_title = (sub or used_key[0] or "").title()
            rel_use = rel or (used_key[1] if used_key else "")
            # special phrasing for parent relation
            if (sub in {"ty", "ty butler"} or "ty" in (sub or "")) and rel_use in {"mom", "mother"}:
                return {"ok": True, "source": "memory", "response": f"{ans} is {subj_title}'s {rel_use}."}
            # general phrasing
            if rel_use in {"mom", "mother", "dad", "father"}:
                return {"ok": True, "source": "memory", "response": f"{ans} is {subj_title}'s {rel_use}."}
            if rel_use in {"full name", "name"}:
                return {"ok": True, "source": "memory", "response": f"{ans} is {subj_title}'s full name."}
            if rel_use in {"hometown"}:
                return {"ok": True, "source": "memory", "response": f"{subj_title} grew up in {ans}."}
            if rel_use in {"birthplace"}:
                return {"ok": True, "source": "memory", "response": f"{subj_title} was born in {ans}."}
            # default sentence
            return {"ok": True, "source": "memory", "response": f"{ans} is {subj_title}'s {rel_use}."}

        # ---------- GPT fallback ----------
        try:
            ok_gpt, reply = gpt_bridge(txt) # must return (bool, str)
        except Exception as e:
